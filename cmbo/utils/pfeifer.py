...
class MatchingProbability(BaseMatchingProbability):
    ...
    """
    def __init__(self, halo_pos, halo_log_mass, mdef="200c",
        ...
        try:
            ...
            raise ImportError(
                "colossus is required for MatchingProbability. "
                "Install it via 'pip install colossus'."
            ) from exc
        ...
        y = mass_function.massFunction(
            x, 0.0, mdef=mdef, model="tinker08", q_out="dndlnM") * np.log(10)
    ...
    def cdf_per_halo(self, refpos, ref_log_mass=None, rmax=50,
        ...
        cdf : 1-dimensional array of shape `(nhalo,)`
            CDF per halo.
        dlogmass : 1-dimensional array of shape `(nhalo,)` or None
            Difference in log mass between the reference and each halo.
            Returned as ``None`` when ``ref_log_mass`` is not provided.
    ...
    def match_halo(self, refpos, ref_log_mass=None, pvalue_threshold=0.005,
        ...
        refpos : 1-dimensional array of shape `(3,)`
            Reference position.
        ref_log_mass : float, optional
            Reference log mass. If omitted, no mass-consistency criterion
            is applied.
        ...
        if dlogmass is not None:
            dlogmass = np.abs(dlogmass[ks])
            mask = (cdf < pvalue_threshold) & (dlogmass < max_absdlogmass)
        else:
            mask = cdf < pvalue_threshold
        ...
        if verbose or len(matches) > 1:
            ...
            for i, k in enumerate(matches):
                ...
                msg = (
                    f"    {i + 1}: CDF = {cdf[k]:.3e}, index = {j}, "
                    f"logM = {logM:.3f} Msun / h, dx = {dx:.3f} Mpc / h"
                )
                if dlogmass is not None:
                    msg += f", |ΔlogM| = {dlogmass[k]:.3f}"
                print(msg + ".")
